...
def draw_text_block(img, arabic_lines, english_lines, ar_font, en_font, alpha):
    ...
    # Center vertically at 45% from top (slightly above center like reference)
    center_y = int(ih * 0.45)
...
def generate_video(texts, translations, audio_path, audio_duration, ayah_durations,
    ...
    ar_font = get_arabic_font(74)  # Large, like reference video
    en_font = get_latin_font(24)
    ...
    try:
        for fi in range(total_frames):
            ...
            # Fade duration: 0.5s fade-in, 0.5s fade-out per ayah (like reference)
            fd_in  = min(0.5, ayah_dur * 0.15)
            fd_out = min(0.5, ayah_dur * 0.15)
            if local_t < fd_in:
                a = local_t / fd_in
            elif local_t > ayah_dur - fd_out:
                a = max(0.0, (ayah_dur - local_t) / fd_out)
            else:
                a = 1.0
            ...
            # Slightly darken the background for better text contrast (like reference)
            # The reference videos are naturally dark/night scenes
